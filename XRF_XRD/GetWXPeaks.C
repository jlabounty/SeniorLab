int GetWXPeaks(
	       TString infile = "data/Scattering_WX_101016_142917.root"
)
{
  TFile *wxFile = TFile::Open(infile); 
  TH1D *wxHraw = new TH1D();
  wxHraw = (TH1D*)wxFile->Get("h");
  TCanvas *c_Wx = new TCanvas();
  wxHraw->Draw();
  ...
  // TH1D *wxHist_K1 = (TH1D*)h->Clone("wxHraw");
  ...
  // TCanvas *c_WxK1 = new TCanvas();
  // /*Fit Pre-Defined Function to Spectrum*/
  // wxHist_K1->Fit("fSpec_K1", "Q", "", 297., 315.);

  // /*Obtain Fit Function from Histogram*/
  // TF1 *fStat_K1 = wxHist_K1->GetFunction("fSpec_K1");
  ...
  // /*===============W-74 K2 Peak===============*/
  // /*Clone Histogram of Spectrum from Data File*/
  // TH1D *wxHist_K2 = (TH1D*)h->Clone("wxHraw");
  // /*Define Spectrum Fit*/
  ...
  // TCanvas *c_WxK2 = new TCanvas();
  // /*Fit Pre-Defined Function to Spectrum*/
  // wxHist_K2->Fit("fSpec_K2", "Q", "", 330., 345.);

  // /*Obtain Fit Function from Histogram*/
  // TF1 *fStat_K2 = wxHist_K2->GetFunction("fSpec_K2");
  ...
  /*================W-74 L1 Peak=================*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *wxHist_L1 = (TH1D*)h->Clone("wxHraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L1 = new TF1("fSpec_L1", "gaus", 335., 355.);
  /*Estimate Parameters of Fit*/
  fSpec_L1->SetParameters(345, 75);
  TCanvas *c_WxL1 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  wxHist_L1->Fit("fSpec_L1", "Q", "", 335., 355.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L1 = wxHist_L1->GetFunction("fSpec_L1");
  ...
  /*===============W-74 L2 Peak===============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *wxHist_L2 = (TH1D*)h->Clone("wxHraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L2 = new TF1("fSpec_L2", "gaus", 385., 400.);
  /*Estimate Parameters of Fit*/
  fSpec_L2->SetParameters(395, 75, 2);
  TCanvas *c_WxL2 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  wxHist_L2->Fit("fSpec_L2", "Q", "", 385., 400.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L2 = wxHist_L2->GetFunction("fSpec_L2");
  ...
  /*===============W-74 L3 Peak===============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *wxHist_L3 = (TH1D*)h->Clone("wxHraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L3 = new TF1("fSpec_L3", "gaus", 455., 470.);
  /*Estimate Parameters of Fit*/
  fSpec_L3->SetParameters(465, 75, 2);
  TCanvas *c_WxL3 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  wxHist_L3->Fit("fSpec_L3", "Q", "", 455., 470.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L3 = wxHist_L3->GetFunction("fSpec_L3");
  ...
  f.Close();
	...
	gStyle->SetOptStat(0);
	std::string title = "^{74}Wx Uncalibrated Spectrum";

	TCanvas *c_WxSpec = new TCanvas("c_WxSpec",title.c_str(),750,750);     //Makes canvas large enough for png printing.
		c_WxSpec->cd();
		c_WxSpec->SetGridx(1);
		c_WxSpec->SetGridy(1);
	//Use blank histogram to set the parameters of the canvas
	TH1F *blankWx = new TH1F("blankWx",title.c_str(),10, 0, 2048);
		blankWx->GetYaxis()->SetRangeUser(0, 600);
		blankWx->GetXaxis()->SetTitle("Bin Number");
		blankWx->GetYaxis()->SetTitle("Photon Count");
		blankWx->GetYaxis()->SetTitleOffset(1.65);
		blankWx->GetXaxis()->SetNdivisions(505);
		blankWx->GetYaxis()->SetNdivisions(505);
		blankWx->SetLineColor(0);
	blankWx->Draw();

	TH1D *hist = new TH1D();
	hist = (TH1D*)wxFile->Get("h");
	hist->Draw("SAME");
	...
	// c_WxSpec->Print("./plots/WxBinnedSpectrum.png");
