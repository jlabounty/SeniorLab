...
Because the strength of the emission lines is also dependent on the concentration of the element in the sample\cite{miller}, it is also possible to determine relative concentrations of elements and not simply their presence. This allows the measurement of trace elements in samples to a great deal of precision, which has applications in a wide range of fields. It can be used in the identification of counterfeit works of art\cite{art1, art2, art3}, as the available pigments differed vastly throughout history. Additionally, older biological materials (i.e. canvas) should have no traces of the radioelements produced by nuclear tests. Floresence studies are also used in determining the diet of ancient hominids from the composition of their bones\cite{art3, arch1, arch2, traces}.
...
To begin this experiment, we needed to characterize the behavior of the detector with respect to the Voltage and Current of the x-ray source, the Voltage of the detector, and the gain settings of the signal amplification. We positioned the detector with no crystal at a slight angle $\approx 2^\circ$. We performed a series of measurements in which we fixed all but one of these variables and varied the others. A subset of these plots can be seen in Figure \ref{fig:xrf_setup_investigation}. These plots helped to inform our decision for the detector/source settings used in the main measurements.
...
\begin{gather}
	...
	K_\beta = 8905.29 ~eV \nonumber
\end{gather}
The value of $\theta$ was determined by using a gaussian fit combined with a polynomial background, which we calculated using ROOT. From the parameters of this fit, we can extract $\theta \pm \delta \theta$. From this we construct the following table for LiF:
\begin{table}[htbp]
	\begin{center}
...
\noindent From this we can see that the values we find for $d$ are consistent within the same crystal. Similar tables were constructed for KCl and NaCl. Using this data, we obtain an average value for the lattice spacing for all three crystals:
