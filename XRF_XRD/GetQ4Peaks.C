int GetQ4Peaks(
	       TString infile = "data/Scattering_Q4_102416_132046.root"
)
{
  TFile *q4File = TFile::Open(infile); 
  TH1D *q4Hraw = new TH1D();
  q4Hraw = (TH1D*)q4File->Get("h");
  TCanvas *c_Q4 = new TCanvas();
  q4Hraw->Draw();
  ...
  TH1D *q4Hist_K1 = (TH1D*)h->Clone("q4Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_K1 = new TF1("fSpec_K1", "gaus", 323., 337.);
  /*Estimate Parameters of Fit*/
  fSpec_K1->SetParameters(330, 10, 2);
  TCanvas *c_Q4K1 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q4Hist_K1->Fit("fSpec_K1", "Q", "", 323., 337.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_K1 = q4Hist_K1->GetFunction("fSpec_K1");
  ...
  /*===========Q4-40 K2 Peak==========*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q4Hist_K2 = (TH1D*)h->Clone("q4Hraw");
  /*Define Spectrum Fit*/
  TF1 *fSpec_K2 = new TF1("fSpec_K2", "gaus", 347., 360.);
  ...
  TCanvas *c_Q4K2 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q4Hist_K2->Fit("fSpec_K2", "Q", "", 347., 360.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_K2 = q4Hist_K2->GetFunction("fSpec_K2");
  ...
  /*==============Q4-40 L1 Peak==============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q4Hist_L1 = (TH1D*)h->Clone("q4Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L1 = new TF1("fSpec_L1", "gaus", 365., 372.);
  /*Estimate Parameters of Fit*/
  fSpec_L1->SetParameters(367, 75, 2);
  TCanvas *c_Q4L1 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q4Hist_L1->Fit("fSpec_L1", "Q", "", 365., 372.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L1 = q4Hist_L1->GetFunction("fSpec_L1");
  ...
  // // /*==============Q4-40 L2 Peak==============*/
  // // /*Clone Histogram of Spectrum from Data File*/
  // // TH1D *q4Hist_L2 = (TH1D*)h->Clone("q4Hraw");
  // // /*Define Spectrum Fit Function*/
  // // TF1 *fSpec_L2 = new TF1("fSpec_L2", "gaus", 283., 297.);
  // // /*Estimate Parameters of Fit*/
  // // fSpec_L2->SetParameters(290, 75, 2);
  // // TCanvas *c_Q4L2 = new TCanvas();
  // // /*Fit Pre-Defined Function to Spectrum*/
  // // q4Hist_L2->Fit("fSpec_L2", "Q", "", 283., 297.);

  // // /*Obtain Fit Function from Histogram*/
  // // TF1 *fStat_L2 = q4Hist_L2->GetFunction("fSpec_L2");
  ...
  // // /*==============Q4-40 L3 Peak==============*/
  // // /*Clone Histogram of Spectrum from Data File*/
  // // TH1D *q4Hist_L3 = (TH1D*)h->Clone("q4Hraw");
  // // /*Define Spectrum Fit Function*/
  // // TF1 *fSpec_L3 = new TF1("fSpec_L3", "gaus", 303., 312.);
  // // /*Estimate Parameters of Fit*/
  // // fSpec_L3->SetParameters(306, 75, 2);
  // // TCanvas *c_Q4L3 = new TCanvas();
  // // /*Fit Pre-Defined Function to Spectrum*/
  // // q4Hist_L3->Fit("fSpec_L3", "Q", "", 303., 312.);

  // // /*Obtain Fit Function from Histogram*/
  // // TF1 *fStat_L3 = q4Hist_L3->GetFunction("fSpec_L3");
  ...
  f.Close();
	...
	gStyle->SetOptStat(0);
	std::string title = "^{40}Q4 Uncalibrated Spectrum";

	TCanvas *c_Q4Spec = new TCanvas("c_Q4Spec",title.c_str(),750,750);     //Makes canvas large enough for png printing.
		c_Q4Spec->cd();
		c_Q4Spec->SetGridx(1);
		c_Q4Spec->SetGridy(1);
	//Use blank histogram to set the parameters of the canvas
	TH1F *blankQ4 = new TH1F("blankQ4",title.c_str(),10, 0, 2048);
		blankQ4->GetYaxis()->SetRangeUser(0, 600);
		blankQ4->GetXaxis()->SetTitle("Bin Number");
		blankQ4->GetYaxis()->SetTitle("Photon Count");
		blankQ4->GetYaxis()->SetTitleOffset(1.65);
		blankQ4->GetXaxis()->SetNdivisions(505);
		blankQ4->GetYaxis()->SetNdivisions(505);
		blankQ4->SetLineColor(0);
	blankQ4->Draw();

	TH1D *hist = new TH1D();
	hist = (TH1D*)q4File->Get("h");
	hist->Draw("SAME");
	...
	// c_Q4Spec->Print("./plots/Q4BinnedSpectrum.png");
