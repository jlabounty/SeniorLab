int GetQ2Peaks(
	       TString infile = "data/Scattering_Q2_101216_121440.root"
)
{
  TFile *q2File = TFile::Open(infile); 
  TH1D *q2Hraw = new TH1D();
  q2Hraw = (TH1D*)q2File->Get("h");
  ...
  TH1D *q2Hist_K1 = (TH1D*)h->Clone("q2Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_K1 = new TF1("fSpec_K1", "gaus", 215., 230.);
  /*Estimate Parameters of Fit*/
  fSpec_K1->SetParameters(225, 10, 2);
  TCanvas *c_Q2K1 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q2Hist_K1->Fit("fSpec_K1", "Q", "", 215., 230.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_K1 = q2Hist_K1->GetFunction("fSpec_K1");
  ...
  /*===========Q2-40 K2 Peak==========*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q2Hist_K2 = (TH1D*)h->Clone("q2Hraw");
  /*Define Spectrum Fit*/
  TF1 *fSpec_K2 = new TF1("fSpec_K2", "gaus", 236., 250.);
  /*Estimate Parameters of Fit Function*/
  // fSpec_K2->SetParNames("Strength", "Mean","Sigma", "Back1", "Back2", "Back3"); 
  fSpec_K2->SetParameters(245., 1000, 2);
  TCanvas *c_Q2K2 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q2Hist_K2->Fit("fSpec_K2", "Q", "", 236., 250.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_K2 = q2Hist_K2->GetFunction("fSpec_K2");
  ...
  /*==============Q2-40 L1 Peak==============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q2Hist_L1 = (TH1D*)h->Clone("q2Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L1 = new TF1("fSpec_L1", "gaus", 255., 270.);
  /*Estimate Parameters of Fit*/
  fSpec_L1->SetParameters(265, 75, 2);
  TCanvas *c_Q2L1 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q2Hist_L1->Fit("fSpec_L1", "Q", "", 255., 270.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L1 = q2Hist_L1->GetFunction("fSpec_L1");
  ...
  /*==============Q2-40 L2 Peak==============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q2Hist_L2 = (TH1D*)h->Clone("q2Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L2 = new TF1("fSpec_L2", "gaus", 283., 297.);
  /*Estimate Parameters of Fit*/
  fSpec_L2->SetParameters(290, 75, 2);
  TCanvas *c_Q2L2 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q2Hist_L2->Fit("fSpec_L2", "Q", "", 283., 297.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L2 = q2Hist_L2->GetFunction("fSpec_L2");
  ...
  /*==============Q2-40 L3 Peak==============*/
  /*Clone Histogram of Spectrum from Data File*/
  TH1D *q2Hist_L3 = (TH1D*)h->Clone("q2Hraw");
  /*Define Spectrum Fit Function*/
  TF1 *fSpec_L3 = new TF1("fSpec_L3", "gaus", 303., 312.);
  /*Estimate Parameters of Fit*/
  fSpec_L3->SetParameters(306, 75, 2);
  TCanvas *c_Q2L3 = new TCanvas();
  /*Fit Pre-Defined Function to Spectrum*/
  q2Hist_L3->Fit("fSpec_L3", "Q", "", 303., 312.);

  /*Obtain Fit Function from Histogram*/
  TF1 *fStat_L3 = q2Hist_L3->GetFunction("fSpec_L3");
  ...
  f.Close();
	...
	gStyle->SetOptStat(0);
	std::string title = "^{40}Q2 Uncalibrated Spectrum";

	TCanvas *c_Q2Spec = new TCanvas("c_Q2Spec",title.c_str(),600,600);     //Makes canvas large enough for png printing.
		c_Q2Spec->cd();
		c_Q2Spec->SetGridx(1);
		c_Q2Spec->SetGridy(1);
	//Use blank histogram to set the parameters of the canvas
	TH1F *blankQ2 = new TH1F("blankQ2",title.c_str(),10, 0, 2048);
		blankQ2->GetYaxis()->SetRangeUser(0, 600);
		blankQ2->GetXaxis()->SetTitle("Bin Number");
		blankQ2->GetYaxis()->SetTitle("Photon Count");
		blankQ2->GetYaxis()->SetTitleOffset(1.65);
		blankQ2->GetXaxis()->SetNdivisions(505);
		blankQ2->GetYaxis()->SetNdivisions(505);
		blankQ2->SetLineColor(0);
	blankQ2->Draw();

	TH1D *hist = new TH1D();
	hist = (TH1D*)q2File->Get("h");
	hist->Draw("SAME");
	...
	// c_Q2Spec->Print("./plots/Q2BinnedSpectrum.png");
